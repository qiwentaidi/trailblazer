...
describe('RiskWorkbench', () => {
  ...
  test('renders recovered private key evidence in the risk drawer', () => {
    render(
      <CodecWorkbenchProvider>
        <RiskWorkbench
          taskId="task-1"
    ...
    expect(screen.getByText('泄露密钥证据')).toBeInTheDocument();
    expect(screen.getByText(/RSA 私钥/)).toBeInTheDocument();
    expect(screen.getByText('MIIE...private-key-material')).toBeInTheDocument();
    expect(screen.getByText(/rsaDecryptNew/)).toBeInTheDocument();
    expect(screen.queryByText(/指纹:/)).not.toBeInTheDocument();
    expect(
      screen.queryByText(/该密钥材料可从目标前端公开资源恢复/),
    ).not.toBeInTheDocument();
  });

  test('renders response plaintext only when static decryption produced a value', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '查看详情' }));
    expect(screen.queryByText('响应解密明文')).not.toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole('button', { name: '查看详情' }));
    expect(screen.getByText('响应解密明文')).toBeInTheDocument();
    expect(screen.getByText('{"code":0,"data":{"id":1}}')).toBeInTheDocument();
  });
