...
// decryptProtocolPayload 使用 trace 材料或手工 key 解密密文
func decryptProtocolPayload(c *gin.Context) {
	...
	result, err := protocoltool.DecryptWithTrace(trace, body.KeyHex, body.Ciphertext)
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to decrypt", "detail": err.Error()})
		return
	}
...
// runtimeDecryptProtocolPayload 强制使用在线 runtime 解密模式
func runtimeDecryptProtocolPayload(c *gin.Context) {
	taskID := c.Param("taskId")
	version, ok := parseTaskVersionQuery(c)
	if !ok {
		return
	...
	resolvedTrace := *trace
	if strings.TrimSpace(body.PageURL) != "" {
		resolvedTrace.PageURL = strings.TrimSpace(body.PageURL)
	}
	if strings.TrimSpace(body.RequestURL) != "" {
		resolvedTrace.RequestURL = strings.TrimSpace(body.RequestURL)
	}

	result, err := protocoltool.RuntimeDecryptWithTrace(&resolvedTrace, body.Ciphertext)
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to runtime decrypt", "detail": err.Error()})
		return
	}
