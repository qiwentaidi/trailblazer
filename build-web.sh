...
WEBASSETS_DIR="$PROJECT_ROOT/webassets/dist"
...
build_frontend() {
  ...
  log_info "同步前端产物到 webassets/dist ..."
  rm -rf "$WEBASSETS_DIR"
...
build_backend_only() {
  ...
  if [ ! -f "$WEBASSETS_DIR/index.html" ]; then
    log_error "找不到 webassets/dist/index.html，请先运行 ./build-web.sh frontend 或 ./build-web.sh all"
    exit 1
  fi
...
用法:
  ./build-web.sh [all|frontend|backend|current|clean|help]

命令:
  all       构建前端并尝试构建所有预设平台
  frontend  仅构建前端并同步到 webassets/dist
