...
func queryStoredStaticProtocolAnalysis(taskID string, version int) (*crawl.StaticProtocolAnalysisResult, bool, error) {
	if database.DB == nil {
		return nil, false, fmt.Errorf("SQLite database not initialized")
	}
	var selected *int
	if version > 0 {
		selected = &version
	}
	documents, err := database.LoadJSONDocuments("static_protocol_analysis", taskID, selected, 1)
	if err != nil {
	...
	var doc staticProtocolAnalysisDocument
	if err := json.Unmarshal(documents[0], &doc); err != nil {
		return nil, false, err
	}
...
func saveStaticProtocolAnalysis(taskID string, version int, result *crawl.StaticProtocolAnalysisResult) error {
	...
	return database.SaveJSONDocument("static_protocol_analysis", taskID, version, fmt.Sprintf("%d", version), result.GeneratedAt, staticProtocolAnalysisDocument{
