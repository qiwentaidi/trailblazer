...
func assetValuesFromSensitiveItems(items []SensitiveItem) []database.AssetValue {
	...
	for _, item := range items {
		...
		values = append(values, database.AssetValue{
			Value:  value,
			Source: nonEmptySourceList(append([]string{item.Source}, item.Sources...)...),
		})
	}
...
func dedupeSensitiveItems(items []SensitiveItem) []SensitiveItem {
	...
	for _, item := range items {
		...
		item.Sources = nonEmptySourceList(append([]string{item.Source}, item.Sources...)...)
		if strings.TrimSpace(item.Source) == "" && len(item.Sources) > 0 {
			item.Source = item.Sources[0]
		}
		if index, ok := seen[key]; ok {
			result[index].Sources = nonEmptySourceList(append(result[index].Sources, append([]string{item.Source}, item.Sources...)...)...)
...
func performAsyncScan(urls []string, taskId string, additionalHighRiskRoutes ...string) {
	version := 0
	...
	if len(additionalHighRiskRoutes) > 0 {
		config.HighRiskRouter = mergeHighRiskRoutes(config.HighRiskRouter, additionalHighRiskRoutes)
	}
	...
	for i, targetURL := range urls {
		...
		if database.DB != nil {
			...
			fmt.Printf("[调试] SQLite 数据库未初始化，跳过保存站点树\n")
		}
	...
	// 去重处理
	allAssets.Email = dedupeSensitiveItems(allAssets.Email)
	allAssets.IDCard = dedupeSensitiveItems(allAssets.IDCard)
	allAssets.Phone = dedupeSensitiveItems(allAssets.Phone)
	allAssets.IPURL = dedupeSensitiveItems(allAssets.IPURL)
	allAssets.Sensitive = dedupeSensitiveItems(allAssets.Sensitive)
...
func mergeHighRiskRoutes(configured, required []string) []string {
	...
	for _, route := range append(configured, required...) {
...
func nodeExists(taskID string, version int, nodeID string) bool {
	...
	nodes, err := database.QuerySiteTreeByTaskID(taskID, version)
	if err != nil {
		return false
	}
	for _, node := range nodes {
