...
build_binary() {
  ...
  log "构建当前平台产物..."
  (cd "$PROJECT_ROOT" && ./build.sh current)
}
...
start_app() {
  ensure_binary
  start_process "prod" "cd '$PROJECT_ROOT' && exec '$BINARY_PATH' web -config '$CONFIG_PATH'"
}
...
start_dev_app() {
  ...
  start_process \
    "dev" \
    "cd '$BACKEND_DIR' && exec go run main.go web -config '$CONFIG_PATH' --frontend-dev-url '$FRONTEND_DEV_URL'"
}
