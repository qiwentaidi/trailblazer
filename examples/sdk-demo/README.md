...
| ② 敏感资产分析 | `sdk.AnalyzeSensitiveAssets(assets, opts)` | 只读、纯静态。分析已采集的 JS，产出脱敏的 Key/Secret、常见 AI API Key、云密钥、邮箱、手机号、身份证、IP/URL 等候选及来源证据 |
...
- `AnalyzeSensitiveAssets` 默认不返回原始敏感值，也不返回源码片段；结果通过来源 URL、字节偏移、规则与脱敏摘要支持受控复核。仅在明确授权的内部流程中设置 `IncludeRawValue: true`。
